...
//import { IFlower } from '../../../react-app-env';
import { useDispatch, useSelector } from 'react-redux';
import { setCurrentFlowers, setAllFlowers, setError, setAllShops, clearCart } from '../../store/actions';
import { getSelectedCart, getAllFlowers, getAllShops, getError } from '../../store/selectors';
import { getFlowers, getShops } from '../../api/api';
...
export function Header() {
  ...
  const flowers = useSelector(getAllFlowers);
  const shops = useSelector(getAllShops);
  const error = useSelector(getError);

  function getFlowersByShop(id) {
    const filteredByShop = flowers.filter(item => item.idshop === id);
    dispatch(clearCart(undefined));
    dispatch(setCurrentFlowers(filteredByShop));
  }
                    ...
                    {shops.map((item) => (
                      ...
                      >
                        ...
                        <div className="flex-auto">
                          <p className="block cursor-pointer font-semibold text-gray-900" onClick={() => getFlowersByShop(item.id)}>
                            {item.name}
                        ...
                        <Disclosure.Panel className="mt-2 space-y-2">
                          {[...shops].map((item) => (
                            <Disclosure.Button
                              ...
                              onClick={() => getFlowersByShop(item.id)}
                            >
                              {item.name}
                            </Disclosure.Button>
                          ))}
